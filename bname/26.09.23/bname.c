...
main(void) {
    ...
    long swap_page_size = sysconf(_SC_PAGE_SIZE);
    long long total_swap_mb = 0;
    if (swap_pages > 0 && swap_page_size > 0) {
        total_swap_mb = ((long long)swap_pages * swap_page_size) / (1024 * 1024);
    }
    //# misc
    char *ver = "26.09.23" ;
    //# Get username
